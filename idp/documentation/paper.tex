	...
	\label{section:proposed_solution}
		...
		\label{section:curve_derivation}
			...
			\label{section:segmentation_notation}
				...
				Let \(m\) be the number of segments that make up the polynomial spline. The parametric curve functions are used with subscript \(i\) when referring to the \(i\)th segment (\(\apply{\phi_i}{t}\)) and without subscript when referring to the whole spline \(\apply{\phi}{t}\). Mapping the parameter \(t\) between the segments and the whole spline works as follows:
				\begin{equation*}
				...
				When using polynomial splines, the optimization domain is a vector consisting of all the coefficients of all the polynomials that make up the spline, denoted \(c\). The parametric curve functions of both the whole spline and each segment are used with subscript \(c\) to indicate their dependence on the coefficient vector (\(\apply{\phi_c}{t}\), \(\apply{{\phi_i}_c}{t}\)) when used in error terms for the optimization.
			...
			\label{section:constant_speed}
				...
				\begin{equation*}
					\max_{t \in \unit} \xa{\apply{\sigma_c}{t} - \hat{\lambda}}
				\end{equation*}
				...
				\begin{equation*}
					\integral{0}{1}{\xp{\apply{\sigma_c}{t} - \hat{\lambda}}^2}{t}
				\end{equation*}
			...
			\label{section:continuity_connections}
				...
				\begin{equation*}
					\begin{gathered}
						\apply{{\phi_i}_c}{1} - \apply{{\phi_{i + 1}}_c}{0}\\
						\apply{{\phi'_i}_c}{1} - \apply{{\phi'_{i + 1}}_c}{0}\\
						\apply{{\phi''_i}_c}{1} - \apply{{\phi''_{i + 1}}_c}{0}
					\end{gathered}
				\end{equation*}
			...
			\label{section:specification_items}
				...
				\begin{equation*}
					\begin{gathered}
						\apply{\phi_c}{\hat{t}_i} - \hat{\phi}_i\\
						\apply{\delta_c}{\hat{t}_i} - \hat{\delta}_i\\
						\apply{\chi_c}{\hat{t}_i} - \hat{\chi}_i
					\end{gathered}
				\end{equation*}
			...
			\label{section:fairness_error}
				...
				\begin{equation*}
					\integral{0}{1}{\apply{\chi'_c}{t}^2}{t}
				\end{equation*}
