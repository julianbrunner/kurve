...
\begin{document}
	...
	\label{section:proposed_solution}
		...
		\label{section:specification-based_curves}
			...
			We think that in order to get the best usability, using a good specification language which does not rely on implementation details has to be the top priority. Together with a good fairness measure, it will then lead to usable curve design tools. Since it is not immediately clear which specification language or which fairness measure is the best, the first step is to move away from the traditional explicit curve construction, and instead use a process that enables the creation of curves based on explicit specifications and a fairness measure.
		...
		\label{section:definitions}
			...
			\subsubsection{Parametric Plane Curves}
		...
		\subsection{Specification Language and Fairness Measure}
		\label{section:specification_language_fairness_measure}
			
			After using the results of the usability analysis to establish the general approach to curve design tools in section \ref{section:specification-based_curves}, it's time to decide on the specifics in order to create an instance of this model. The first steps are to choose both a specification language and a fairness measure.
		...
		\label{section:nonlinear_optimization_polynomial_curves}
			...
			\label{section:optimization_problem}
				...
				In the following sections, we will introduce the optimization goals that enable us to use nonlinear optimization for building curves based on specifications using the specification language and fairness measure established in section \ref{section:specification_language_fairness_measure}. We will also state how they are expressed using constraint function and bounds as well as objective function.
