...
\renewcommand{\vector}[2]{\begin{pmatrix}#1\\\linebreak{}#2\end{pmatrix}}
...
\begin{document}
	...
	\label{section:proposed_solution}
		...
		\label{section:parametric_plane_curves}
			...
			\begin{align*}
				\overline{\cdot} & : \mathbb{R}^2 \rightarrow \mathbb{R}^2     & \overline{v}      & = \vector{-v_2}{v_1}                      && \text{orthogonal vector}\\
			...
			\begin{align*}
				...
				\kappa           & : \left[0,1\right] \rightarrow \mathbb{R}   & \apply{\kappa}{t} & = \frac{\apply{\theta'}{t}}{\apply{s}{t}} = \frac{\apply{p''}{t}^\mathrm{T}\overline{\apply{p'}{t}}}{\apply{s}{t}^3} && \text{curvature}
			\end{align*}
		...
		\label{section:nonlinear_optimization_polynomial_curves}
			...
			\label{section:optimization_problem}
				...
				Since we chose to give the position of a specification item as a fraction of the curve's total arc length, we need a function mapping covered arc length to parameter values (the inverse to the function \(l\) defined in section \ref{section:parametric_plane_curves}). Unfortunately, this function isn't very well suited to be used as a term in a smooth opimization problem even for simple curves like polynomials. Thus, an additional optimization goal is to find parametric curves where this function is approximately linear. This property is equivaent to the speed of the curve being approximately constant and has additional useful consequences, such as the second derivative of the curve being orthogonal to the first derivative. Since the chosen specification language already specifies the total arc length of the curve, and the parameter runs through the unit range, the target value for the speed is simply the total arc length of the curve (\(l^*\)), leading to the following error term:
				\begin{displaymath}
					\int_{0}^{1}\left(\apply{s}{t} - l^*\right)^2\;\mathrm{d}t
				\end{displaymath}
