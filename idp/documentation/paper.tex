		...
		\label{section:nonlinear_optimization_polynomial_curves}
			...
			\label{section:optimization_problem}
				...
				\begin{align*}
					...
					v & : \left[0,1\right] \rightarrow \mathbb{R}^2, & v & = p' && \text{(velocity)}\\
					a & : \left[0,1\right] \rightarrow \mathbb{R}^2, & a & = v' && \text{(acceleration)}
				\end{align*}

				The first problem we face is that polynomials will in general not be in natural parametrization (that is, a parametrization where the parameter \(t\) is equal to the covered arc length of the curve up to the point \(p\left(t\right)\)). Since we chose to give the position of a specification item as a fraction of the curve's total arc length, it would be very advantageous to have a parametrization where the parameter is at least proportional to the covered arc length of the curve. This property is equivalent to the first derivative of the curve (velocity) having constant euclidean norm (\(\|p'\left(t\right)\|\) is constant) and has additional useful consequences, such as the second derivative of the curve (acceleration) being orthogonal to the velocity. Thus, one of the objectives of the optimization is to make sure that the speed of the curve is constant.
