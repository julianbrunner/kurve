	...
	\label{section:evaluation}
		...
		\label{section:usability_analysis}
			...
			\subsubsection{Expressiveness of the Specification Language}
				...
				%         compare to using elliptical arcs in inkscape
				%       a good way to pull curves in place when they don't have direction specifications
				%       benign behavior
				%   doesn't allow *not* expressing length, may pose a problem
			...
			%   familiar differential geometric properties
			% human ability to `speak' the specification language to the software
			%   familiar differential geometric properties
			% smoothness of curves selected by the fairness measure
			%   very good, thanks to MVC
			% minimality of curves selected by the fairness measure
			%   not guaranteed by optimization, but may usually be obtained by pushing the curve into a different optimum
			% software's ability to derive curves from descriptions
			%   optimization is slow and sometimes unstable, still needs some work before it becomes practical
			%   local optima
			%   bad stability, infeasible for numeric optimization
			%   performance
			%   adjustment of segment count and degree by the user is not desirable
